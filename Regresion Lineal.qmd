...
```{r}
spx_csv <- read.csv("^SPX.csv")
```

```{r}
spx_csv$Date = ymd(spx_csv$Date)
```
...
```{r}
train_spx <- spx %>% select(Adj.Close) %>% filter_index("2023 Jan" ~ "2023 Jun")
test_spx <- spx %>% select(Adj.Close) %>% filter_index("2023 Jul" ~ "2023 Dec")
```

```{r}
train_amzn <- amzn %>% select(Adj.Close) %>% filter_index("2023 Jan" ~ "2023 Jun")
test_amzn <- amzn %>% select(Adj.Close) %>% filter_index("2023 Jul" ~ "2023 Dec")
train_amzn <- na.omit(train_amzn)
test_amzn <- na.omit(test_amzn)
```


```{r}
train_spx <- add_column(train_spx, train_amzn$Adj.Close) #append remesas
colnames(train_spx)[3] = "amzn" 
colnames(train_spx)[1] = "spx"

```

```{r}
amzn %>% 
  ggplot(aes(x = spx$Adj.Close, y = Adj.Close)) + 
  labs(y = "S&P 500",
       x = "AMZN") + 
  geom_point() +
...
plot_lm = augment(fit_lm) |>
  ggplot(aes(x = Date)) +
  geom_line(aes(y = amzn, colour = "reales")) +
...
```{r}
augment(fit_lm) |> filter((Date) > 2023) %>%
  ggplot(aes(x = Date)) +
  geom_line(aes(y = amzn, colour = "reales")) +
  geom_line(aes(y = .fitted, colour = "ajustados")) +
  labs(y = NULL,
...
```
```{r}
fit_lm_filled <- fit_lm
```

```{r}
fit_lm_filled <- na.omit(fit_lm)
```

```{r}
feasts::gg_tsresiduals(fit_lm_filled)
```
